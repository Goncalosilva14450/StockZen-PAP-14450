...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['saida'])) {
        ...
        $queryStock = "SELECT stock_quantity, name FROM products WHERE product_id=$product_id";
        $resultStock = mysqli_query($con, $queryStock);
        $row = mysqli_fetch_assoc($resultStock);
        $currentStock = $row['stock_quantity'];
        $product_name = $row['name']; // Nome do produto
        ...
        if ($currentStock >= $saida_quantity) {
            ...
            // Inserir o histórico de saída, incluindo o nome do produto
            $historicoQuery = "INSERT INTO stock_movement (product_name, quantity, observation, date) 
            VALUES ('$product_name', $saida_quantity, '$motivo', NOW())";

            if (!mysqli_query($con, $historicoQuery)) {
                die("Erro ao registrar histórico de saída: " . mysqli_error($con));
            }
    ...
    <div id="editModal" class="modal">
        <div class="modal-content">
            ...
            <form id="editForm" method="POST">
                ...
                <label for="modalStock">Quantidade em Stock:</label>
                <input type="number" id="modalStock" name="stock_quantity" required>
                <div class="modal-actions">
                    <button type="submit" name="edit" class="btn btn-primary">Salvar</button>
                    <button type="button" class="btn btn-danger close">Cancelar</button>
                </div>
            </form>
        </div>
    </div>
    ...
    <div id="saidaModal" class="modal">
        <div class="modal-content">
            ...
            <form id="saidaForm" method="POST">
                ...
                <input type="number" id="saidaQuantity" name="saida_quantity" required>
                <label for="motivo">Motivo da Saída:</label>
                <input type="text" id="motivo" name="motivo" required>
                <div class="modal-actions">
                    <button type="submit" name="saida" class="btn btn-primary">Registrar Saída</button>
                    <button type="button" class="btn btn-danger close">Cancelar</button>
                </div>
            </form>
        </div>
    ...
    <script>
        ...
        document.querySelectorAll('.editBtn').forEach(button => {
            button.addEventListener('click', function () {
                ...
                document.getElementById('modalStock').value = productStock;

                document.getElementById('editModal').style.display = 'block';
            });
        });

        // Funcionalidade do Modal de Saída
        document.querySelectorAll('.saidaBtn').forEach(button => {
            button.addEventListener('click', function () {
                const productId = this.getAttribute('data-id');
                const productStock = this.getAttribute('data-stock');

                document.getElementById('saidaProductId').value = productId;
                document.getElementById('saidaQuantity').value = '';
        ...
        // Fechar o modal de saída se o usuário clicar fora da caixa de conteúdo
        window.addEventListener('click', function (event) {
            if (event.target === document.getElementById('editModal') || event.target === document.getElementById('saidaModal')) {
